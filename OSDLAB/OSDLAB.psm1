...
function Receive-ADK10
{
	...
	[OutputType([psobject])]
	param (
		[ValidateSet('ADK10_RTM', 'ADK10_1511','ADK_1607','ADK_1703')]
		$ADK,
	...
	$ADK10_1607 = "http://download.microsoft.com/download/9/A/E/9AE69DD5-BA93-44E0-864E-180F5E700AB4/adk/adksetup.exe"
	$ADK10_1703 = "http://download.microsoft.com/download/5/D/9/5D915042-FCAA-4859-A1C3-29E198690493/adk/adksetup.exe"
	...
	Switch ($ADK)
	{
		"ADK10_RTM"
		{
			...
			if (!(Test-Path "$ADK_BASEVER\ADK10_RTM\adksetup.exe"))
			{
				...
				If ($DownloadContent.ispresent)
				{
					...
					Show-BalloonTip -Text "Download Full $ADK content" -Icon Info -Timeout 10 -Title "Downloading..."
		...
		"ADK10_1607"
		{
			$ADK_BASEVER = $ADK
			$ADK_BASEDir = Join-Path $Product_Dir $ADK_BASEVER
			if (!(Test-Path "$ADK_BASEVER\ADK10_1607\adksetup.exe"))
			{
				foreach ($url in ($ADK10_1607))
				{
					$FileName = Split-Path -Leaf -Path $Url
					Write-Verbose "Testing $FileName in $ADK_BASEDir"
					if (!(test-path  "$ADK_BASEDir\$FileName"))
				...
				{
					Write-Verbose "Download $ADK Installation"
					$FileName = Split-Path -Leaf $ADK10_1607
					Show-BalloonTip -Text "Download Full $ADK content" -Icon Info -Timeout 10 -Title "Downloading..."
					Start-Process "$ADK_BASEDir\$FileName" -ArgumentList "/quiet /Layout $ADK_BASEDir" -Wait
				}
			}
		...
		"ADK10_1703"
		{
			$ADK_BASEVER = $ADK
			$ADK_BASEDir = Join-Path $Product_Dir $ADK_BASEVER
			if (!(Test-Path "$ADK_BASEVER\ADK10_1703\adksetup.exe"))
			{
				foreach ($url in ($ADK10_1703))
				{
					$FileName = Split-Path -Leaf -Path $Url
					Write-Verbose "Testing $FileName in $ADK_BASEDir"
					if (!(test-path  "$ADK_BASEDir\$FileName"))
				...
				{
					Write-Verbose "Download $ADK Installation"
					$FileName = Split-Path -Leaf $ADK10_1703
					Show-BalloonTip -Text "Download Full $ADK content" -Icon Info -Timeout 10 -Title "Downloading..."
					Start-Process "$ADK_BASEDir\$FileName" -ArgumentList "/quiet /Layout $ADK_BASEDir" -Wait
				}
			}
...
function Receive-MDTFiles
{
	...
	[OutputType([psobject])]
	param (
		[ValidateSet('MDT2013U1', 'MDT2013U2','MDT')]
		$MDTVersions,
		[ValidateSet('C:\HydrationCM\Source\Downloads')]
		[String]$Destination,
	...
	$MDT_F1 = "https://download.microsoft.com/download/3/3/9/339BE62D-B4B8-4956-B58D-73C4685FC492/MicrosoftDeploymentToolkit_x64.msi"
	$MDT_F2 = "https://download.microsoft.com/download/3/3/9/339BE62D-B4B8-4956-B58D-73C4685FC492/MicrosoftDeploymentToolkit_x86.msi"
	$Product_Dir = Join-Path $Destination $Product_Dir
	Write-Verbose "Destination: $Product_Dir"
	if (!(Test-Path $Product_Dir))
	{
	...
	Switch ($MDTVersions)
	{
		...
		"MDT"
		{
			$MDT_BASEVER = $MDTVersions
			$MDT_BASEDir = Join-Path $Product_Dir $MDT_BASEVER
			if (!(Test-Path "$MDT_BASEVER\2013U1\MicrosoftDeploymentToolkit_x64.msi"))
			{
				foreach ($url in ($MDT_F1, $MDT_F2))
				{
					$FileName = Split-Path -Leaf -Path $Url
					Write-Verbose "Testing $FileName in $MDT_BASEDir"
					if (!(test-path  "$MDT_BASEDir\$FileName"))
...
function Receive-SCCM
{
	...
	Switch ($SCCMVersions)
	{
		"1511"
		{
			...
			if (!(Test-Path "$SCCM_BASEVER\1511\SC_Configmgr_1511.exe"))
			{
				foreach ($url in ($SCCM1511))
				{
					$FileName = Split-Path -Leaf -Path $Url
		...
		"1606"
		{
			$SCCM_BASEVER = $SCCMVersions
			$SCCM_BASEDir = Join-Path $Product_Dir $SCCM_BASEVER
			if (!(Test-Path "$SCCM_BASEVER\1606\SC_Configmgr_SCEP_1606.exe"))
			{
				foreach ($url in ($SCCM1606))
				{
					$FileName = Split-Path -Leaf -Path $Url
					Write-Verbose "Testing $FileName in $SCCM_BASEDir"
					if (!(test-path  "$SCCM_BASEDir\$FileName"))
...
function Receive-WindowsClient_Eval
{
	[CmdletBinding(DefaultParametersetName = "1",
				   SupportsShouldProcess = $true,
				   ConfirmImpact = "Medium")]
	...
	Switch ($OSVersions)
	{
		"Win10LSTBEVAL"
		{
			$OS_BASEVER = $OSVersions
			$OS_BASEDir = Join-Path $Product_Dir $OS_BASEVER
			if (!(Test-Path "$OS_BASEVER\Win10LSTBEVAL\10240.16384.150709-1700.TH1_CLIENTENTERPRISE_S_EVAL_X64FRE_FR-FR.ISO"))
			{
				foreach ($url in ($OSFile))
				{
					$FileName = Split-Path -Leaf -Path $Url
		...
		"Win101511EVAL"
		{
			$OS_BASEVER = $OSVersions
			$OS_BASEDir = Join-Path $Product_Dir $OS_BASEVER
			if (!(Test-Path "$OS_BASEVER\Win101511EVAL\10586.0.151029-1700.TH2_RELEASE_CLIENTENTERPRISEEVAL_OEMRET_X64FRE_FR-FR.ISO"))
			{
		...
		"Win81EntFR"
		{
			$OS_BASEVER = $OSVersions
			$OS_BASEDir = Join-Path $Product_Dir $OS_BASEVER
			if (!(Test-Path "$OS_BASEVER\Win81EntFR\9600.17050.WINBLUE_REFRESH.140317-1640_X64FRE_ENTERPRISE_EVAL_FR-FR-IR3_CENA_X64FREE_FR-FR_DV9.ISO"))
			{
				foreach ($url in ($OSFileWin81))
				{
					$FileName = Split-Path -Leaf -Path $Url
					Write-Verbose "Testing $FileName in $OS_BASEDir"
					if (!(test-path  "$OS_BASEDir\$FileName"))
					{
						Show-BalloonTip -Text " Trying Download Windows Windows 8.1 Entreprise" -Icon Info -Timeout 10 -Title "Downloading..."
						if (!(Receive-ToolsBitsFile -DownLoadUrl $URL -destination "$OS_BASEDir\$FileName"))
						{
							Show-BalloonTip -Text " Error Donwloading File $url , Please check connectivity" -Icon Error -Timeout 10 -Title "Downloading..."
							exit
...
function Receive-Windows10ADMX
{
	...
	$ADMXFile = "https://download.microsoft.com/download/D/7/4/D74DD144-DBC7-496E-A09F-A2DE8E9D38D1/Windows_%2010_Creators_Update_ADMX.msi"
